...
pub const SaDbPlan3Info = extern struct {
    written: u64,
    total: u64,
    first_predicate: u64,
    first_total: u64,
...
fn outputU64sAllowEmpty(ptr: ?[*]u64, len: u64) ?[]u64 {
    if (len > @as(u64, @intCast(std.math.maxInt(usize)))) return null;
    const n: usize = @intCast(len);
    if (n == 0) return empty_output_u64s[0..];
    const p = ptr orelse return null;
    return p[0..n];
}

fn outputI64sAllowEmpty(ptr: ?[*]i64, len: u64) ?[]i64 {
    if (len > @as(u64, @intCast(std.math.maxInt(usize)))) return null;
    const n: usize = @intCast(len);
    if (n == 0) return empty_output_i64s[0..];
    const p = ptr orelse return null;
    return p[0..n];
}
...
fn fillPlan3Info(out_info: ?*SaDbPlan3Info, result: table.Plan3RowsResult) u32 {
    const slot = out_info orelse return SA_DB_ERR_INVALID_ARGUMENT;
    slot.* = .{
        .written = result.written,
        .total = result.total,
        .first_predicate = result.first_predicate,
        .first_total = result.first_total,
        .second_predicate = result.second_predicate,
        .second_total = result.second_total,
        .third_predicate = result.third_predicate,
        .third_total = result.third_total,
    };
    return SA_DB_OK;
}
...
fn groupSortByFromAbi(value: u32) ?table.GroupSortBy {
    return switch (value) {
        0 => .key,
        1 => .count,
        2 => .sum,
...
pub export fn sa_db_group_sum_i64_by_u64_handle(
    handle: ?*anyopaque,
    group_column_index: u64,
    sum_column_index: u64,
    offset: u64,
    limit: u64,
    out_keys_ptr: ?[*]u64,
    out_counts_ptr: ?[*]u64,
    ...
    if (sum_column_index > @as(u64, @intCast(std.math.maxInt(usize)))) return SA_DB_ERR_INVALID_ARGUMENT;
    var gpa = std.heap.GeneralPurposeAllocator(.{}){};
    defer _ = gpa.deinit();
    const snapshot = acquireReadSnapshot(handle) orelse return SA_DB_ERR_INVALID_ARGUMENT;
    defer releaseReadSnapshot(snapshot);
    const result = table.snapshotGroupSumI64ByU64(gpa.allocator(), snapshot, @intCast(group_column_index), @intCast(sum_column_index), offset, limit, out_keys, out_counts, out_sums) catch |err| return tableStatus(err);
    written_slot.* = result.written;
    total_slot.* = result.total;
    return SA_DB_OK;
}

pub export fn sa_db_group_rows_sum_i64_by_u64_handle(
    handle: ?*anyopaque,
    group_column_index: u64,
    sum_column_index: u64,
    in_rows_ptr: ?[*]const u64,
    ...
    const snapshot = acquireReadSnapshot(handle) orelse return SA_DB_ERR_INVALID_ARGUMENT;
    defer releaseReadSnapshot(snapshot);
    const result = table.snapshotGroupRowsSumI64ByU64(gpa.allocator(), snapshot, @intCast(group_column_index), @intCast(sum_column_index), in_rows, offset, limit, out_keys, out_counts, out_sums) catch |err| return tableStatus(err);
    written_slot.* = result.written;
    total_slot.* = result.total;
    return SA_DB_OK;
}

pub export fn sa_db_group_sum_i64_by_u64_sorted_handle(
    handle: ?*anyopaque,
    group_column_index: u64,
    sum_column_index: u64,
    sort_by: u32,
    ...
    const snapshot = acquireReadSnapshot(handle) orelse return SA_DB_ERR_INVALID_ARGUMENT;
    defer releaseReadSnapshot(snapshot);
    const result = table.snapshotGroupSumI64ByU64Sorted(gpa.allocator(), snapshot, @intCast(group_column_index), @intCast(sum_column_index), group_sort_by, sort_descending, offset, limit, out_keys, out_counts, out_sums) catch |err| return tableStatus(err);
    written_slot.* = result.written;
    total_slot.* = result.total;
    return SA_DB_OK;
}

pub export fn sa_db_group_rows_sum_i64_by_u64_sorted_handle(
    handle: ?*anyopaque,
    group_column_index: u64,
    sum_column_index: u64,
    in_rows_ptr: ?[*]const u64,
...
pub export fn sa_db_filter_rows_u64_range_null_bitmap_handle(
    handle: ?*anyopaque,
    column_index: u64,
    in_rows_ptr: ?[*]const u64,
    in_rows_len: u64,
    ...
    const snapshot = acquireReadSnapshot(handle) orelse return SA_DB_ERR_INVALID_ARGUMENT;
    defer releaseReadSnapshot(snapshot);
    const result = table.snapshotFilterRowsU64RangeNullBitmap(snapshot, @intCast(column_index), in_rows, min_value, max_value, null_bitmap, want_null != 0, offset, limit, rows) catch |err| return tableStatus(err);
    written_slot.* = result.written;
    total_slot.* = result.total;
    return SA_DB_OK;
}

pub export fn sa_db_filter_rows_i64_range_null_bitmap_handle(
    handle: ?*anyopaque,
    column_index: u64,
    in_rows_ptr: ?[*]const u64,
    in_rows_len: u64,
    ...
    const snapshot = acquireReadSnapshot(handle) orelse return SA_DB_ERR_INVALID_ARGUMENT;
    defer releaseReadSnapshot(snapshot);
    const result = table.snapshotFilterRowsI64RangeNullBitmap(snapshot, @intCast(column_index), in_rows, min_value, max_value, null_bitmap, want_null != 0, offset, limit, rows) catch |err| return tableStatus(err);
    written_slot.* = result.written;
    total_slot.* = result.total;
    return SA_DB_OK;
}
...
pub export fn sa_db_plan_u64_i64_blob_eq_handle(
    handle: ?*anyopaque,
    u64_column_index: u64,
    u64_min_value: u64,
    u64_max_value: u64,
    ...
    var gpa = std.heap.GeneralPurposeAllocator(.{}){};
    defer _ = gpa.deinit();
    const result = table.snapshotPlanU64I64BlobEqRows(
        gpa.allocator(),
        snapshot,
        @intCast(u64_column_index),
        u64_min_value,
...
pub export fn sa_db_plan_u64_i64_bool_handle(
    handle: ?*anyopaque,
    u64_column_index: u64,
    u64_min_value: u64,
    u64_max_value: u64,
    ...
    if (u64_column_index > @as(u64, @intCast(std.math.maxInt(usize)))) return SA_DB_ERR_INVALID_ARGUMENT;
    if (i64_column_index > @as(u64, @intCast(std.math.maxInt(usize)))) return SA_DB_ERR_INVALID_ARGUMENT;
    if (bool_column_index > @as(u64, @intCast(std.math.maxInt(usize)))) return SA_DB_ERR_INVALID_ARGUMENT;
    const expected_bool = boolFromAbi(expected) orelse return SA_DB_ERR_INVALID_ARGUMENT;
    const snapshot = acquireReadSnapshot(handle) orelse return SA_DB_ERR_INVALID_ARGUMENT;
    defer releaseReadSnapshot(snapshot);
    var gpa = std.heap.GeneralPurposeAllocator(.{}){};
    defer _ = gpa.deinit();
    const result = table.snapshotPlanU64I64BoolRows(
        gpa.allocator(),
        snapshot,
        @intCast(u64_column_index),
        u64_min_value,
...
pub export fn sa_db_plan_u64_i64_i64_ranges_handle(
    handle: ?*anyopaque,
    u64_column_index: u64,
    u64_min_value: u64,
    u64_max_value: u64,
    first_i64_column_index: u64,
    first_i64_min_value: i64,
    first_i64_max_value: i64,
    second_i64_column_index: u64,
    ...
    const rows = outputU64s(out_rows_ptr, out_rows_len) orelse return SA_DB_ERR_INVALID_ARGUMENT;
    if (u64_column_index > @as(u64, @intCast(std.math.maxInt(usize)))) return SA_DB_ERR_INVALID_ARGUMENT;
    if (first_i64_column_index > @as(u64, @intCast(std.math.maxInt(usize)))) return SA_DB_ERR_INVALID_ARGUMENT;
    if (second_i64_column_index > @as(u64, @intCast(std.math.maxInt(usize)))) return SA_DB_ERR_INVALID_ARGUMENT;
    const snapshot = acquireReadSnapshot(handle) orelse return SA_DB_ERR_INVALID_ARGUMENT;
    defer releaseReadSnapshot(snapshot);
    var gpa = std.heap.GeneralPurposeAllocator(.{}){};
    defer _ = gpa.deinit();
    const result = table.snapshotPlanU64I64I64RangeRows(
        gpa.allocator(),
        snapshot,
        @intCast(u64_column_index),
        u64_min_value,
        u64_max_value,
        @intCast(first_i64_column_index),
        first_i64_min_value,
        first_i64_max_value,
        @intCast(second_i64_column_index),
...
pub export fn sa_db_filter_rows_decimal_i64_range_null_bitmap_handle(
    handle: ?*anyopaque,
    column_index: u64,
    in_rows_ptr: ?[*]const u64,
    in_rows_len: u64,
    ...
    status = sa_db_decimal_from_parts(max_negative, max_whole, max_fraction, scale, &max_value);
    if (status != SA_DB_OK) return status;
    return sa_db_filter_rows_i64_range_null_bitmap_handle(handle, column_index, in_rows_ptr, in_rows_len, min_value, max_value, null_bitmap_ptr, null_bitmap_len, want_null, offset, limit, out_rows_ptr, out_rows_len, out_written, out_total);
}
...
pub export fn sa_db_filter_rows_date_range_null_bitmap_handle(
    handle: ?*anyopaque,
    column_index: u64,
    in_rows_ptr: ?[*]const u64,
    in_rows_len: u64,
...
pub export fn sa_db_filter_rows_timestamp_ms_range_null_bitmap_handle(
    handle: ?*anyopaque,
    column_index: u64,
    in_rows_ptr: ?[*]const u64,
    in_rows_len: u64,
...
pub export fn sa_db_filter_rows_timestamp_us_range_null_bitmap_handle(
    handle: ?*anyopaque,
    column_index: u64,
    in_rows_ptr: ?[*]const u64,
    in_rows_len: u64,
...
pub export fn sa_db_project_rows_handle(
    ...
    const row_indices = inputU64sAllowEmpty(row_indices_ptr, row_indices_len) orelse return SA_DB_ERR_INVALID_ARGUMENT;
    ...
    const out_bytes = outputBytesAllowEmpty(out_bytes_ptr, out_bytes_len) orelse return SA_DB_ERR_INVALID_ARGUMENT;
...
test "db SA ABI filters candidate rows for ERP predicates" {
    ...
    try std.testing.expectEqual(SA_DB_OK, sa_db_null_bitmap_set(&null_bitmap, null_bitmap.len, 1, 1));
    try std.testing.expectEqual(SA_DB_OK, sa_db_null_bitmap_set(&null_bitmap, null_bitmap.len, 5, 1));

    try std.testing.expectEqual(SA_DB_OK, sa_db_filter_rows_u64_range_null_bitmap_handle(handle, 2, &original_candidate_rows, candidate_written, 2, 2, &null_bitmap, null_bitmap.len, 1, 0, filtered.len, &filtered, filtered.len, &written, &total));
    try std.testing.expectEqual(@as(u64, 2), total);
    try std.testing.expectEqual(@as(u64, 2), written);
    try std.testing.expectEqual(@as(u64, 1), filtered[0]);
    try std.testing.expectEqual(@as(u64, 5), filtered[1]);

    try std.testing.expectEqual(SA_DB_OK, sa_db_filter_rows_i64_range_null_bitmap_handle(handle, 4, &rows, status_written, 1500, 5500, &null_bitmap, null_bitmap.len, 0, 0, filtered.len, &filtered, filtered.len, &written, &total));
    try std.testing.expectEqual(@as(u64, 1), total);
    try std.testing.expectEqual(@as(u64, 1), written);
    try std.testing.expectEqual(@as(u64, 2), filtered[0]);
    ...
    var plan3_info: SaDbPlan3Info = undefined;
    try std.testing.expectEqual(SA_DB_OK, sa_db_plan_u64_i64_blob_eq_handle(handle, 2, 2, 2, 1, 0, 20, 13, "doc_type".ptr, "doc_type".len, "invoice".ptr, "invoice".len, 0, planned_rows.len, &planned_rows, planned_rows.len, &plan3_info));
    ...
    try std.testing.expectEqual(@as(u64, 3), plan3_info.second_total);
    ...
    try std.testing.expectEqual(SA_DB_OK, sa_db_plan_u64_i64_blob_eq_handle(handle, 2, 2, 2, 1, -5, 25, 13, "doc_type".ptr, "doc_type".len, "order".ptr, "order".len, 1, 1, &planned_rows, planned_rows.len, &plan3_info));
    ...
    try std.testing.expectEqual(SA_DB_OK, sa_db_plan_u64_i64_blob_eq_handle(handle, 2, 1, 1, 1, -5, 25, 13, "doc_type".ptr, "doc_type".len, "invoice".ptr, "invoice".len, 0, planned_rows.len, &planned_rows, planned_rows.len, &plan3_info));
    try std.testing.expectEqual(@as(u64, 1), plan3_info.first_predicate);
    try std.testing.expectEqual(@as(u64, 2), plan3_info.first_total);
    try std.testing.expectEqual(@as(u64, 3), plan3_info.second_predicate);
    try std.testing.expectEqual(@as(u64, 3), plan3_info.second_total);
    try std.testing.expectEqual(@as(u64, 2), plan3_info.third_predicate);
    ...
    try std.testing.expectEqual(SA_DB_OK, sa_db_plan_u64_i64_bool_handle(handle, 2, 2, 2, 1, -5, 25, 3, 0, 0, planned_rows.len, &planned_rows, planned_rows.len, &plan3_info));
    try std.testing.expectEqual(@as(u64, 3), plan3_info.first_predicate);
    try std.testing.expectEqual(@as(u64, 1), plan3_info.first_total);
    try std.testing.expectEqual(@as(u64, 1), plan3_info.second_predicate);
    try std.testing.expectEqual(@as(u64, 4), plan3_info.second_total);
    try std.testing.expectEqual(@as(u64, 2), plan3_info.third_predicate);
    try std.testing.expectEqual(@as(u64, 6), plan3_info.third_total);
    ...
    try std.testing.expectEqual(SA_DB_OK, sa_db_plan_u64_i64_bool_handle(handle, 2, 1, 1, 1, -5, 25, 3, 1, 1, 1, &planned_rows, planned_rows.len, &plan3_info));
    try std.testing.expectEqual(@as(u64, 1), plan3_info.first_predicate);
    try std.testing.expectEqual(@as(u64, 2), plan3_info.first_total);
    try std.testing.expectEqual(@as(u64, 3), plan3_info.second_predicate);
    try std.testing.expectEqual(@as(u64, 5), plan3_info.second_total);
    try std.testing.expectEqual(@as(u64, 2), plan3_info.third_predicate);
    try std.testing.expectEqual(@as(u64, 6), plan3_info.third_total);
    try std.testing.expectEqual(@as(u64, 2), plan3_info.total);
    try std.testing.expectEqual(@as(u64, 1), plan3_info.written);
    ...
    try std.testing.expectEqual(SA_DB_OK, sa_db_plan_u64_i64_i64_ranges_handle(handle, 2, 2, 2, 1, -5, 25, 4, 1500, 3500, 0, planned_rows.len, &planned_rows, planned_rows.len, &plan3_info));
    try std.testing.expectEqual(@as(u64, 3), plan3_info.first_predicate);
    try std.testing.expectEqual(@as(u64, 2), plan3_info.first_total);
    try std.testing.expectEqual(@as(u64, 1), plan3_info.second_predicate);
    try std.testing.expectEqual(@as(u64, 4), plan3_info.second_total);
    ...
    try std.testing.expectEqual(SA_DB_OK, sa_db_plan_u64_i64_i64_ranges_handle(handle, 2, 2, 2, 1, 0, 20, 4, 1500, 5500, 0, planned_rows.len, &planned_rows, planned_rows.len, &plan3_info));
    try std.testing.expectEqual(@as(u64, 2), plan3_info.first_predicate);
    try std.testing.expectEqual(@as(u64, 3), plan3_info.first_total);
    try std.testing.expectEqual(@as(u64, 1), plan3_info.second_predicate);
    try std.testing.expectEqual(@as(u64, 4), plan3_info.second_total);
    ...
    try std.testing.expectEqual(SA_DB_OK, sa_db_plan_u64_i64_i64_ranges_handle(handle, 2, 1, 1, 1, -5, 25, 4, 0, 6000, 1, 1, &planned_rows, planned_rows.len, &plan3_info));
    try std.testing.expectEqual(@as(u64, 1), plan3_info.first_predicate);
    try std.testing.expectEqual(@as(u64, 2), plan3_info.first_total);
    try std.testing.expectEqual(@as(u64, 3), plan3_info.second_predicate);
    try std.testing.expectEqual(@as(u64, 5), plan3_info.second_total);
...
test "db SA ABI groups signed sums by u64 keys" {
    var original_cwd = try std.fs.cwd().openDir(".", .{});
    defer original_cwd.close();
    var tmp = std.testing.tmpDir(.{ .iterate = true });
    defer tmp.cleanup();
    ...
    const root = ".";
    const table_name = "group_amounts";
    const schema_source =
        \\#def MAX_ROWS = 8
    ...
    ;
    var info: SaDbTableInfo = undefined;
    try std.testing.expectEqual(SA_DB_OK, sa_db_init_schema(root.ptr, root.len, "group_amounts.sadb-schema".ptr, "group_amounts.sadb-schema".len, schema_source.ptr, schema_source.len, &info));

    var customers = [_]u64{ 1, 2, 1, 3, 2 };
    var amounts = [_]i64{ 10, -5, 20, 7, 15 };
    const cols = [_]SaDbColumnInput{
        .{ .data = @ptrCast(&customers), .len = @sizeOf(@TypeOf(customers)) },
        .{ .data = @ptrCast(&amounts), .len = @sizeOf(@TypeOf(amounts)) },
    };
    try std.testing.expectEqual(SA_DB_OK, sa_db_ingest_columns(root.ptr, root.len, table_name.ptr, table_name.len, customers.len, &cols, cols.len, &info));
    ...
    try std.testing.expectEqual(@as(i64, 7), sums[2]);

    keys = .{ 99, 99, 99 };
    counts = .{ 99, 99, 99 };
    sums = .{ 99, 99, 99 };
    try std.testing.expectEqual(SA_DB_OK, sa_db_group_sum_i64_by_u64_handle(handle, 0, 1, 1, 1, &keys, &counts, &sums, keys.len, &written, &total));
    try std.testing.expectEqual(@as(u64, 3), total);
    try std.testing.expectEqual(@as(u64, 1), written);
    ...
    counts = .{ 99, 99, 99 };
    sums = .{ 99, 99, 99 };
    try std.testing.expectEqual(SA_DB_OK, sa_db_group_sum_i64_by_u64_sorted_handle(handle, 0, 1, 2, 1, 0, 2, &keys, &counts, &sums, keys.len, &written, &total));
    try std.testing.expectEqual(@as(u64, 3), total);
    try std.testing.expectEqual(@as(u64, 2), written);
    try std.testing.expectEqual(@as(u64, 1), keys[0]);
    try std.testing.expectEqual(@as(u64, 2), counts[0]);
    ...
    counts = .{ 99, 99, 99 };
    sums = .{ 99, 99, 99 };
    try std.testing.expectEqual(SA_DB_OK, sa_db_group_sum_i64_by_u64_sorted_handle(handle, 0, 1, 0, 1, 0, 2, &keys, &counts, &sums, keys.len, &written, &total));
    try std.testing.expectEqual(@as(u64, 3), total);
    try std.testing.expectEqual(@as(u64, 2), written);
    try std.testing.expectEqual(@as(u64, 3), keys[0]);
    try std.testing.expectEqual(@as(u64, 1), counts[0]);
    try std.testing.expectEqual(@as(i64, 7), sums[0]);
    try std.testing.expectEqual(@as(u64, 2), keys[1]);
    try std.testing.expectEqual(@as(u64, 2), counts[1]);
    try std.testing.expectEqual(@as(i64, 10), sums[1]);
    ...
    const invalid_rows = [_]u64{99};
    written = 99;
    total = 99;
    try std.testing.expectEqual(SA_DB_ERR_INVALID_FORMAT, sa_db_group_rows_sum_i64_by_u64_handle(handle, 0, 1, &invalid_rows, invalid_rows.len, 0, keys.len, &keys, &counts, &sums, keys.len, &written, &total));
    try std.testing.expectEqual(@as(u64, 0), written);
    try std.testing.expectEqual(@as(u64, 0), total);
    try std.testing.expectEqual(SA_DB_OK, sa_db_close_read_table(handle));
}
...
test "db SA ABI creates ingests updates and scans raw columns" {
    ...
    try std.testing.expectEqual(@as(u64, 32), projected_required);
    projected_written = 99;
    projected_required = 99;
    try std.testing.expectEqual(SA_DB_OK, sa_db_project_rows_handle(handle, null, 0, &project_columns, project_columns.len, null, 0, &projected_written, &projected_required));
    try std.testing.expectEqual(@as(u64, 0), projected_written);
    try std.testing.expectEqual(@as(u64, 0), projected_required);
