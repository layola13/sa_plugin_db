...
`sa_db_blob_put` / `DB_BLOB_PUT` 是通用可变长字段层：行内只保存 8 字节 `blob_handle`，命名 blob store 保存真实 bytes。blob artifact 会随 snapshot、restore、verify、recover、lock、unlock 和 remove 一起处理，并记录 whole-file 与 block-level hash。这样普通行查询、索引和投影仍保持固定宽度，备注、说明、地址、外部文件 key 等字段可以放在行外存储。`DB_FILTER_BLOB_EQ_HANDLE` 和 `DB_FILTER_BLOB_CONTAINS_HANDLE` 则提供 read handle 上的第一版列表过滤能力，后续再把高频文本/blob 条件推进到二级索引。

这仍不是完整的 SQLite 级 ACID/WAL，也还没有多表事务。面向中小型 ERP 的下一阶段需要继续扩大故障注入矩阵、补齐可选 WAL、更多主键/二级索引形态、string/blob 二级索引/搜索，以及 SA 编写的 ERP 场景 benchmark。当前 primitive 类型元数据、逻辑类型 encode/decode helper、decimal/date/timestamp typed range、`u8/i8/u16/i16/u32/i32/u64/i64/f32/f64` 索引点查/范围/比较读取、`u64/i64` 范围查询的 null bitmap 分页前过滤、`u64` 唯一索引、`u64_pair` 组合键点查与分页、低基数字符串字典及读句柄解码、`blob_handle` 可变长 bytes/text 存储、读句柄解码及 exact/contains 过滤、范围分页、批量投影读取、固定宽度行 insert/upsert、带恢复 marker 的单表批量事务、列段/索引/字典/blob store block checksum、按 row-index 或唯一 key get、按唯一 key delete、单表跨进程写锁已经可作为主键语义、类型感知行编码和列表页读取的第一步。
