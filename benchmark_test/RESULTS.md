...
当前 ERP workflow benchmark 已有 db 和 SQLite 两侧程序，但这里只记录一次本机验证输出，还不是 5 轮中位数结论。

db 插件：

```text
db_erp_init_ns=133220315
db_erp_prepare_ns=1755198
db_erp_ingest_ns=136730840
db_erp_index_ns=204384813
db_erp_order_lines_ns=495179
db_erp_project_ns=6753
db_erp_customer_orders_ns=17715
db_erp_due_invoices_ns=12064
db_erp_inventory_ns=9739
db_erp_verify_ns=158764310
order_line_total=4
customer_order_total=8
due_invoice_total=1280
inventory_move_total=32
project_written_rows=4
paid_invoice_count=4096
```

SQLite：

```text
sqlite_erp_init_ns=1572515
sqlite_erp_ingest_ns=123912620
sqlite_erp_index_ns=30483049
sqlite_erp_order_lines_ns=83321
sqlite_erp_project_ns=50738
sqlite_erp_customer_orders_ns=38245
sqlite_erp_due_invoices_ns=387410
sqlite_erp_inventory_ns=59927
sqlite_erp_verify_ns=43385526
order_line_total=4
customer_order_total=8
due_invoice_total=1280
inventory_move_total=32
...
这个 workload 的价值不是单个数字，而是固定住中小型 ERP 常见访问形状：订单明细、客户订单、到期发票、库存流水、状态计数和列表 projection。后续优化并发查询、mmap snapshot、索引规划或 SQLite ERP 对照时，应继续跑这个基准，并以 5 轮中位数更新上面的单次输出。
