...
当前 ERP workflow benchmark 先建立 db 插件侧基线，SQLite 对照还未补齐，因此下面不是 db-vs-SQLite 结论。一次本机验证输出如下：

```text
db_erp_init_ns=158142140
db_erp_prepare_ns=1698555
db_erp_ingest_ns=147025353
db_erp_index_ns=218648018
db_erp_order_lines_ns=22955
db_erp_project_ns=6744
db_erp_customer_orders_ns=17273
db_erp_due_invoices_ns=9038
db_erp_inventory_ns=9770
db_erp_verify_ns=168311103
order_line_total=4
...
这个 workload 的价值不是单个数字，而是固定住中小型 ERP 常见访问形状：订单明细、客户订单、到期发票、库存流水、状态计数和列表 projection。后续优化并发查询、mmap snapshot、索引规划或 SQLite ERP 对照时，应继续跑这个基准。
