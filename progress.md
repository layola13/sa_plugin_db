...
- `sa build-exe benchmark_test/db_blob_planner_smoke.sa -o benchmark_test/db_blob_planner_smoke.out --no-incremental` plus the resulting smoke binary pass and cover `DB_PLAN_U64_BLOB_EQ_HANDLE`, `DB_PLAN_I64_BLOB_EQ_HANDLE`, `SaDbPlanInfo`, smaller-side choice, range-first/blob-first plans, and pagination through `db.sal`.
...
- `nm -D zig-out/lib/libdb.so | rg 'sa_db_plan_(u64_i64|u64_u64|i64_i64)_ranges_handle$|sa_db_plan_(u64|i64)_blob_eq_handle$'` verifies the planner ABI symbols are exported.
