...
- `sa build-exe benchmark_test/db_planner_smoke.sa -o benchmark_test/db_planner_smoke.out --no-incremental` plus the resulting smoke binary pass and cover `DB_PLAN_U64_I64_RANGES_HANDLE`, `SaDbPlanInfo`, smaller-side choice, and pagination through `db.sal`.
...
- `nm -D zig-out/lib/libdb.so | rg 'sa_db_plan_u64_i64_ranges_handle$'` verifies the new planner ABI symbol is exported.
