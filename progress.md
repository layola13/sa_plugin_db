...
- Current status: `16 / 16` original qmod/CLI features completed (`100.0%`). Since that milestone, the plugin has moved into a development ABI phase with `db.sai` and `db.sal` exported through `sap.json` for SA-facing read handles, typed queries, row writes, dictionaries, blob stores, and ERP planner helpers. Row-list intersection, union, exclusion, and candidate text/blob filtering are now exposed for indexed AND/OR/not-in and text-search list composition. Strict fixed-width row update/upsert/delete by unique `u64`, `u8`, `i8`, `u16`, `i16`, `u32`, `i32`, and `i64` keys is available both directly and inside single-table transactions, and read handles can copy full rows by unique typed integer keys (`u64`, `i64`, `u32`, `i32`, `u8`, `i8`, `u16`, `i16`) for ERP form loads. Unique `(u64, u64)` plus unique `(u64, i64)` composite-key upsert/update/delete is now available for ERP child rows and signed date/timestamp second-key shapes such as order lines, product/warehouse balances, customer/order-day lists, and status/due-day lists. Transaction-scoped dictionary intern is available through `sa_db_tx_dict_intern` / `DB_TX_DICT_INTERN`, allowing ERP status/category labels and the rows that reference them to commit or roll back together.
